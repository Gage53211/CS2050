...
public class Program7 
{
   ...
   public static int[] intInsertionSort (int[] array)
   {
      ...
      for (int selected = 0; selected < sortedArray.length; selected++) 
      {
         
         previous = selected;
         
         while (previous> 0 && sortedArray[previous - 1] > sortedArray[previous])
         {
            hold = sortedArray[previous - 1];
            sortedArray[previous - 1] = sortedArray[previous];
            sortedArray[previous] = hold;
   ...
   public static String[] stringInsertionSort (String[] array)
   {
      ...
      for (int selected = 0; selected < sortedArray.length; selected++) 
      {
         ...
         while (previous > 0 && (sortedArray[previous - 1].compareTo(sortedArray[previous]) > 0))
         {
            hold = sortedArray[previous - 1];
            sortedArray[previous - 1] = sortedArray[previous];
            sortedArray[previous] = hold;
      ...
      try
      {
         ...
         BufferedWriter writer = new BufferedWriter(new FileWriter ("results.txt"));
            ...
            for (int i = 0; i < 20000; i++)
            {
               ...
               intInsertionSortArray[i] = parsedInteger;
            ...
            for (int i = 0; i < 10000; i++)
            {
               ...
               stringInsertionSortArray[i] = readString;
         ...
         // time calculation for Number insertion sort
         starttime = System.nanoTime();
         intInsertionSortArray = intInsertionSort(intInsertionSortArray);
         endtime = System.nanoTime();
 
         writer.write ("\nNumber insertion sort took " + (endtime - starttime) / Billion + " seconds to finish with 20,000 integers\n");
         ...
         // time calculation for String insertion sort
         starttime = System.nanoTime();
         stringInsertionSortArray =  stringInsertionSort(stringInsertionSortArray);
         endtime = System.nanoTime();
 
         writer.write ("\nString insertion sort took " + (endtime - starttime) / Billion + " seconds to finish with 10,000 Strings");
